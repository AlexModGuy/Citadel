...
//lower priority to be loaded after terrablender merges its rules
@Mixin(value = NoiseBasedChunkGenerator.class, priority = 1500)
public class NoiseBasedChunkGeneratorMixin {

    @Shadow public Holder<NoiseGeneratorSettings> settings;

    @Unique
    private SurfaceRules.RuleSource getOrCreateMergedSurfaceRules(SurfaceRules.RuleSource ruleSource) {
        if(((Object)settings.get()) instanceof NoiseGeneratorSettingsAccessor accessor){
            accessor.afterLoadOtherSurfaceRules(settings.get().surfaceRule());
            return accessor.getRuleSource();
        }
        return settings.get().surfaceRule();
    }
